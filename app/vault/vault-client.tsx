...
export function VaultClient({ initialAuthData }: VaultClientProps) {
  ...
  const canAccess = (item: VaultItem) => {
    ...
    const userTierLevel = tierLevels[currentProfile.tier as keyof typeof tierLevels] || 1
    const requiredTierLevel = tierLevels[item.tier_required as keyof typeof tierLevels] || 1

    return userTierLevel >= requiredTierLevel && (currentProfile.coins || 0) >= item.coin_cost
  }

  const unlockItem = async (item: VaultItem) => {
    if (!currentProfile || !canAccess(item)) return
    ...
    try {
      ...
      if (item.coin_cost > 0) {
        const { error: updateError } = await supabase
          .from("users")
          .update({ coins: currentProfile.coins - item.coin_cost })
                  ...
                  <div className="flex gap-2">
                    {canAccess(item) || unlockedItems.has(item.id) ? (
                      ...
                      <Button
                        ...
                        disabled={!currentProfile || (currentProfile.coins || 0) < item.coin_cost}
                      >
