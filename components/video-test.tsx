...
export function VideoTest() {
  ...
  useEffect(() => {
    ...
    const testVideo = async () => {
      try {
        const response = await fetch("/videos/erigga-hero-video.mp4", { method: "HEAD" })
        if (response.ok) {
          setVideoExists(true)
        } else {
  ...
  return (
    <div className="fixed bottom-4 right-4 bg-black/80 text-white p-4 rounded-lg text-sm z-50">
      ...
      <p>Path: /videos/erigga-hero-video.mp4</p>
      ...
      <div className="mt-2">
        ...
        <video width="100" height="60" controls muted className="border border-gray-500">
          <source src="/videos/erigga-hero-video.mp4" type="video/mp4" />
          <source
