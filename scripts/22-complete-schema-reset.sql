...
-- to fix all the reported errors
...
-- Step 2: Create the main users table with proper UUID handling
...
-- Step 3: Create community categories
...
-- Step 4: Create community posts with proper foreign keys
CREATE TABLE public.community_posts (
    id SERIAL PRIMARY KEY,
    user_id UUID NOT NULL REFERENCES public.users(id) ON DELETE CASCADE,
    category_id INTEGER NOT NULL REFERENCES public.community_categories(id) ON DELETE RESTRICT,
...
-- Step 5: Create community comments
...
-- Step 10: Create coin transactions
...
-- Step 12: Create automatic user creation function
...
-- Step 13: Create trigger for automatic user creation
DROP TRIGGER IF EXISTS on_auth_user_created ON auth.users;
CREATE TRIGGER on_auth_user_created
    AFTER INSERT ON auth.users
    FOR EACH ROW EXECUTE FUNCTION public.handle_new_user();

-- Step 14: Create vote handling function with proper UUID handling
...
BEGIN
    ...
    ELSE
        ...
        VALUES (v_voter_id, -p_coin_amount, 'vote', 'Post vote');
        
        INSERT INTO public.coin_transactions (user_id, amount, transaction_type, description)
        VALUES (v_post_creator_id, p_coin_amount, 'reward', 'Post vote received');
...
-- Step 15: Create stats update functions
...
BEGIN
    IF TG_TABLE_NAME = 'community_posts' THEN
        ...
        ELSIF TG_OP = 'DELETE' THEN
            ...
            UPDATE public.community_categories SET post_count = post_count - 1 WHERE id = OLD.category_id;
        END IF;
...
-- Step 18: Create RLS policies (drop existing first to avoid conflicts)
DO $$
BEGIN
    ...
    DROP POLICY IF EXISTS "Users can edit own posts" ON public.community_posts;
    ...
    DROP POLICY IF EXISTS "Users can edit own comments" ON public.community_comments;
    DROP POLICY IF EXISTS "Users can view votes" ON public.community_post_votes;
    DROP POLICY IF EXISTS "Users can vote" ON public.community_post_votes;
...
-- Step 21: Final verification and summary
DO $$
DECLARE
    table_count INTEGER;
...
BEGIN
    ...
    AND trigger_name IN ('on_auth_user_created', 'trigger_update_post_stats', 'trigger_update_comment_stats', 'trigger_update_follow_stats');
    ...
    IF table_count < 9 THEN
        RAISE WARNING 'Expected 9 tables, but only % were created', table_count;
    END IF;
    ...
    IF category_count < 7 THEN
        RAISE WARNING 'Expected 7 categories, but only % were created', category_count;
    END IF;
END $$;
