-- =====================================================
-- FINAL CLEAN SETUP - Fixed version
-- =====================================================
...
-- Step 20: Create test users and content
INSERT INTO public.users (
    auth_user_id,
    username,
    full_name,
...
#Freestyle #EriggaBars #Warri #PaperBoiVibes',
    ...
    18,
    89
FROM public.users u WHERE u.username = 'warriking';
...
#TheErigma #Motivation #DeepLyrics #RealMusic',
    ...
    12,
    67
FROM public.users u WHERE u.username = 'naijafan';
...
-- Step 22: Create sample comments
INSERT INTO public.community_comments (post_id, user_id, content)
SELECT 
    p.id,
    u.id,
    'First! 🔥 So excited for this community!'
FROM public.community_posts p, public.users u 
WHERE p.content LIKE 'Welcome to the official%' AND u.username = 'warriking';

INSERT INTO public.community_comments (post_id, user_id, content)
SELECT 
    p.id,
    u.id,
    'This is amazing! My favorite track is "Motivation"!'
FROM public.community_posts p, public.users u 
WHERE p.content LIKE 'Welcome to the official%' AND u.username = 'naijafan';

INSERT INTO public.community_comments (post_id, user_id, content)
SELECT 
    p.id,
    u.id,
    'Those bars are fire! 🔥 Keep pushing the culture!'
FROM public.community_posts p, public.users u 
WHERE p.content LIKE 'Just dropped some fire%' AND u.username = 'eriggaofficial';

INSERT INTO public.community_comments (post_id, user_id, content)
SELECT 
    p.id,
    u.id,
    'Yo this is sick! The flow reminds me of "Paper Boi"!'
FROM public.community_posts p, public.users u 
WHERE p.content LIKE 'Just dropped some fire%' AND u.username = 'naijafan';

-- Step 23: Create sample votes
INSERT INTO public.community_post_votes (post_id, user_id)
SELECT p.id, u.id
FROM public.community_posts p, public.users u 
WHERE p.content LIKE 'Welcome to the official%' AND u.username = 'warriking';
...
-- Step 24: Create sample follows
INSERT INTO public.user_follows (follower_id, following_id)
SELECT u1.id, u2.id
FROM public.users u1, public.users u2
WHERE u1.username = 'warriking' AND u2.username = 'eriggaofficial';

INSERT INTO public.user_follows (follower_id, following_id)
SELECT u1.id, u2.id
FROM public.users u1, public.users u2
WHERE u1.username = 'naijafan' AND u2.username = 'eriggaofficial';

INSERT INTO public.user_follows (follower_id, following_id)
SELECT u1.id, u2.id
FROM public.users u1, public.users u2
WHERE u1.username = 'naijafan' AND u2.username = 'warriking';

-- Step 25: Update counts to match actual data
UPDATE public.community_posts 
