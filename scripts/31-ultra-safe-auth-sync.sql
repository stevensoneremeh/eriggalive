-- =====================================================
-- ULTRA SAFE AUTH USERS SYNC - NO FOREIGN KEY ISSUES
-- This script avoids foreign key constraint problems
-- =====================================================
...
BEGIN
    ...
    IF NOT column_exists THEN
        ...
        EXCEPTION WHEN OTHERS THEN
            RAISE NOTICE 'Could not add unique constraint to referral_code: %', SQLERRM;
        END;
    END IF;
    
    -- Check if referred_by column exists (WITHOUT foreign key constraint)
    SELECT EXISTS (
        SELECT 1 FROM information_schema.columns 
        WHERE table_schema = 'public' 
        AND table_name = 'users' 
    ...
    IF NOT column_exists THEN
        RAISE NOTICE 'Adding missing referred_by column (without foreign key)...';
        -- Add column without foreign key constraint to avoid issues
...
-- Create a minimal function to sync auth users to public users table
CREATE OR REPLACE FUNCTION sync_auth_user_minimal(auth_user_record auth.users)
RETURNS void
LANGUAGE plpgsql
SECURITY DEFINER
AS $$
...
BEGIN
    ...
    -- Insert with only essential columns to avoid constraint issues
...
BEGIN
    RAISE NOTICE 'Starting minimal auth user sync...';
    
    FOR auth_user_record IN SELECT * FROM auth.users LOOP
        BEGIN
            PERFORM sync_auth_user_minimal(auth_user_record);
...
CREATE OR REPLACE FUNCTION handle_new_auth_user_minimal()
RETURNS trigger
LANGUAGE plpgsql
SECURITY DEFINER
AS $$
BEGIN
    ...
    PERFORM sync_auth_user_minimal(NEW);
    RETURN NEW;
END;
$$;
...
CREATE TRIGGER on_auth_user_created
    ...
    FOR EACH ROW
    EXECUTE FUNCTION handle_new_auth_user_minimal();

-- Create a function to get or create user profile
CREATE OR REPLACE FUNCTION get_or_create_user_profile_safe(user_auth_id uuid)
...
DECLARE
    sample_user record;
BEGIN
    RAISE NOTICE '=== SAMPLE SYNCED USERS ===';
    FOR sample_user IN 
        SELECT u.id, u.username, u.email, u.auth_user_id::text as auth_id
        FROM public.users u 
        ORDER BY u.created_at DESC 
        LIMIT 5
    LOOP
        RAISE NOTICE 'User: % (%) - Auth ID: %', sample_user.username, sample_user.email, sample_user.auth_id;
    END LOOP;
END $$;
