...
-- Create user_profiles table (main user table)
...
-- Create freebies table (already created above but ensuring it exists)
...
CREATE POLICY "Users can create posts" ON public.community_posts
    FOR INSERT WITH CHECK (
        EXISTS (
            SELECT 1 FROM public.user_profiles
            WHERE auth_user_id = auth.uid()
            AND id = community_posts.user_id
        )
    );
...
CREATE POLICY "Users can manage their own votes" ON public.community_post_votes
    FOR ALL USING (
        EXISTS (
            SELECT 1 FROM public.user_profiles
            WHERE auth_user_id = auth.uid()
            AND id = community_post_votes.user_id
        )
    );
...
CREATE POLICY "Users can create comments" ON public.community_comments
    FOR INSERT WITH CHECK (
        EXISTS (
            SELECT 1 FROM public.user_profiles
            WHERE auth_user_id = auth.uid()
            AND id = community_comments.user_id
        )
    );
...
CREATE POLICY "Users can manage their own comment likes" ON public.community_comment_likes
    FOR ALL USING (
        EXISTS (
            SELECT 1 FROM public.user_profiles
            WHERE auth_user_id = auth.uid()
            AND id = community_comment_likes.user_id
        )
    );
...
CREATE POLICY "Users can view their own transactions" ON public.coin_transactions
    FOR SELECT USING (
        EXISTS (
            SELECT 1 FROM public.user_profiles
            WHERE auth_user_id = auth.uid()
            AND id = coin_transactions.user_id
        )
    );
...
-- Insert sample freebies (if not already inserted)
