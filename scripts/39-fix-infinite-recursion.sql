...
CREATE POLICY "profiles_update_own" ON user_profiles FOR UPDATE USING (user_id = auth.uid());
CREATE POLICY "profiles_insert_own" ON user_profiles FOR INSERT WITH CHECK (user_id = auth.uid());

-- For profiles table (if it uses id column linked to auth.users)
CREATE POLICY "profiles_select_own" ON profiles FOR SELECT USING (id = auth.uid());
CREATE POLICY "profiles_update_own" ON profiles FOR UPDATE USING (id = auth.uid());
CREATE POLICY "profiles_insert_own" ON profiles FOR INSERT WITH CHECK (id = auth.uid());

-- For users_new table
CREATE POLICY "users_new_select_own" ON users_new FOR SELECT USING (auth_user_id = auth.uid());
CREATE POLICY "users_new_update_own" ON users_new FOR UPDATE USING (auth_user_id = auth.uid());
CREATE POLICY "users_new_insert_own" ON users_new FOR INSERT WITH CHECK (auth_user_id = auth.uid());
